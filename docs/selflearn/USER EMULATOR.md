...
> **Эмулятор — это ещё один MaaS Agent, а не просто клиент.**

### Архитектура: два агента с памятью

```
┌────────────────────────────────────────────────────────────────┐
│                    Диалог двух агентов                         │
│                                                                │
│   MaaS Agent A                      MaaS Agent B               │
│   (Ученик)                          (Учитель)                  │
│   ┌──────────────────┐              ┌──────────────────┐       │
...
1. **MaaS = Агент** (ассистенты + память), а не просто модуль памяти
2. **Оба агента накапливают память** — Ученик помнит что спрашивал, Учитель помнит что отвечал
3. **Разница только в meta-prompt** — он определяет роль (Ученик/Учитель)
4. **Self-Learning оценивает обоих** — фокус на качестве памяти
5. **Симуляция времени** — имитируем timestamps для тестирования памяти за месяцы/годы
