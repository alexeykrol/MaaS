...
| `meta_student` | Мета-промпт для роли Ученика | §10 |
| `meta_teacher` | Мета-промпт для роли Учителя | §10 |
...
## 10. Meta-Prompt (Role Prompt) — определение роли агента

### 10.1. Что является импакт-фактором

**Артефакт:** мета-промпт, который определяет роль агента в диалоге.

В системе эмуляции и обучения есть два агента:
- **Ученик** — задаёт вопросы, изучает тему
- **Учитель** — отвечает, объясняет

Разница между ними — **только мета-промпт**.

### 10.2. Механизм влияния

Мета-промпт определяет:
- Как агент формулирует запросы/ответы
- Какую роль занимает в диалоге
- Как использует свою память

Это влияет на **качество диалога** и, следовательно, на качество данных для обучения.

### 10.3. Метрики

- **Dialog Quality** — насколько диалог реалистичен и полезен
- **Role Consistency** — держит ли агент свою роль
- **Memory Usage** — как эффективно агент использует память в своей роли

### 10.4. Целевой артефакт MaaS (куда Tuner записывает)

**Таблица:** `system_prompts`
**Ключи:** `meta_student`, `meta_teacher`

```sql
-- Мета-промпт для Ученика
UPDATE system_prompts
SET content = 'Ты ученик, который изучает тему. Задавай вопросы, уточняй...'
WHERE prompt_type = 'meta_student' AND is_active = true;

-- Мета-промпт для Учителя
UPDATE system_prompts
SET content = 'Ты учитель, который объясняет тему. Отвечай понятно...'
WHERE prompt_type = 'meta_teacher' AND is_active = true;
```

**Tuner записывает:** UPDATE текста промпта в таблице `system_prompts`.

### 10.5. Примеры мета-промптов

**Ученик:**
```
Ты изучаешь тему [X].
Твоя цель — разобраться в теме через диалог.
Задавай вопросы, проси уточнения, фиксируй что понял.
Используй свою память о прошлых разговорах.
```

**Учитель:**
```
Ты объясняешь тему [X].
Твоя цель — помочь собеседнику разобраться.
Отвечай понятно, приводи примеры.
Используй свою память о том, что уже обсуждали.
```

---

## 11. Сводка по приоритетам
...
10. **Meta-Prompt (`meta_student`, `meta_teacher`)**
    Определяет роль агента в диалоге (Ученик/Учитель).
